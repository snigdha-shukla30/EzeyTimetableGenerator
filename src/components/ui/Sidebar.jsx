...
{ id: "Batch", label: "Batch", path: "/generate" },
  ...
  // URL se automatically detect karo active menu
  ...
  const computedActiveMenu = getActiveMenu();
  const computedActiveSubMenu = getActiveSubMenu();

  // Timetable submenu auto open karo agar submenu path pe ho
  useEffect(() => {
    if (computedActiveMenu === "timetable" && computedActiveSubMenu) {
      setTimetableOpen(true);
    } else {
      setTimetableOpen(false);
    }
    if (setActiveMenu) setActiveMenu(computedActiveMenu);
  }, [location.pathname]);
  ...
  const renderItem = (item) => {
    ...
    const isActive = computedActiveMenu === item.id;
