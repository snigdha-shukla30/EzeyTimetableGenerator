...
export default function Faculty({ searchQuery = "", refreshTrigger = 0 }) {
  ...
  const fetchFaculties = async () => {
    try {
      ...
      if (response.success && response.data) setFacultyList(response.data);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (facultyId) => {
    ...
    if (response.success) fetchFaculties();
  };
              ...
              <td className="py-5 text-center">
                {editingId === f._id ? (
                  ...
                  <button
                    ...
                    className="text-[13px] font-medium text-[#1A8FE3]"
                  >
                ...
                {editingId === f._id ? (
                  <div className="flex justify-center gap-2">
                    <button
                      ...
                      className="text-[#1A8FE3] text-[12px]"
                    >
                      Save
                    </button>
                    <button
                      onClick={handleCancelEdit}
                      className="text-[#8A96A8] text-[12px]"
                    >
                      Cancel
                    </button>
                  </div>
